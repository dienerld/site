...
<template>
  <div ref="tracingBeamRef" :class="`relative w-full  mx-auto sm:-left-1.5 md:-left-0 h-full  ${props.class})`">
    <div class="absolute -left-4 top-3 md:-left-12">
      ...
      <svg
        :viewBox="`0 0 20 ${svgHeight}`" width="20"
        :height="svgHeight"
        class="ml-4 block" aria-hidden="true"
      >
        <path
          :d="`M 1 0V -36 l 18 24 V ${svgHeight * 0.8} l -18 24V ${svgHeight}`" fill="none"
          stroke="#9091A0"
          stroke-opacity="0.16"
        />
        <path
          :d="`M 1 0V -36 l 18 24 V ${svgHeight * 0.8} l -18 24V ${svgHeight}`" fill="none"
          stroke="url(#gradient)"
          stroke-width="1.25" class="motion-reduce:hidden"
        />
        <defs>
          <linearGradient
    ...
    <div ref="tracingBeamContentRef" class="ml-10 md:ml-0">
      <slot />
    </div>
  </div>
</template>
